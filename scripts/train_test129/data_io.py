...
def cal_Psepos(seqlist, PDB_DF_dir, Dataset_dir, psepos, seqanno):
    seq_CA_pos = {}
    seq_centroid = {}
    ...
    for seq_id in tqdm(seqlist):
            ...
            if psepos == 'CA':
                with open(Dataset_dir + '/' + '_psepos_' + psepos + '.pkl', 'wb') as f:
...
def cal_PSSM(seq_list, pssm_dir, feature_dir):
    ...
    with open(feature_dir + '/{}_PSSM.pkl', 'wb') as f:
        pickle.dump(nor_pssm_dict, f)
    return


def cal_HMM(seq_list, hmm_dir, feature_dir):
    hmm_dict = {}
    for seqid in seq_list:
        file = seqid + '.hhm'
    ...
    with open(feature_dir + '/{}_HMM.pkl', 'wb') as f:
...
def cal_DSSP(seq_list, dssp_dir, feature_dir):
    ...
    with open(feature_dir + '/{}_SS.pkl', 'wb') as f:
...
def PDBResidueFeature(seqlist, PDB_DF_dir, feature_dir, residue_feature_list, feature_combine, atomfea):
    for fea in residue_feature_list:
        with open(feature_dir + '/' + '_{}.pkl'.format(fea), 'rb') as f:
...
if __name__ == '__main__':
    ...
    trainingset_dict = {'PDNA': 'Train.txt'}

    testset_dict = {'PDNA': '129_Test.txt'}

 
    Dataset_dir = '/home/mijia/GLM-EP/data'
    ...
    cal_Psepos(seqlist, PDB_DF_dir, Dataset_dir, psepos,seqanno)
    ...
    cal_PSSM(seqlist, Dataset_dir + '/PSSM', Dataset_dir)
  
    cal_DSSP(seqlist, Dataset_dir + '/SS', Dataset_dir)

    PDBResidueFeature(seqlist, PDB_DF_dir, Dataset_dir,  feature_list, feature_combine, atomfea)

    root_dir = Dataset_dir + '/' + '_{}_dist{}_{}'.format(psepos, dist, feature_combine)
