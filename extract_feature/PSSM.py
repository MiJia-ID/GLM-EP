...
#                 continue
#             elements = line.split()
#             if len(elements) >= 22:  # Ensure there are enough columns
#                 try:
...
#
# psiblast -query test.fasta -db /home/mijia/app/pssm/databases/swissprot -num_iterations 3 -out_ascii_pssm pssm.txt -out psiblast_output.txt
# #pssm.txt就是PSSM矩阵
# pssm_matrix = parse_pssm('/home/mijia/app/pssm/databases/pssm.txt')
# print(pssm_matrix.shape)  # Should print (L, 20)
# print(pssm_matrix)
...
def run_psiblast(fasta_file, pssm_output_dir):
    ...
    if not os.path.exists(pssm_file):
        ...
        cmd = [
            "psiblast",
            "-query", fasta_file,
            "-db", "/home/mijia/app/pssm/databases/swissprot",
            "-num_iterations", "3",
...
def main():
    fasta_file = "/home/mijia/EGPDI/data/DNA_Test_181_Extracted.fasta"  # 多序列FASTA文件路径
    split_dir = "/home/mijia/EGPDI/data/PSSM/split_data"  # 存放拆分后单序列FASTA文件的目录
    pssm_output_dir = "/home/mijia/EGPDI/data/PSSM"  # 保存PSSM矩阵的输出目录
